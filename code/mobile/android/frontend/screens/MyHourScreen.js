...
export default function MyHourScreen() {
  ...
  const remainingHours = workedHours
    ? totalHours - parseInt(workedHours, 10)
    : totalHours;
  ...
  return (
    <View style={styles.container}>
      ...
      <View
        style={[
          styles.circle,
          { borderColor: getRemainingColor(remainingPercentage) },
        ]}
      >
        <Text style={styles.remainingHours}>{`${remainingHours}h left`}</Text>
      </View>
      <Text style={styles.percentage}>{`${remainingPercentage.toFixed(
        0
      )}%`}</Text>
    </View>
  );
